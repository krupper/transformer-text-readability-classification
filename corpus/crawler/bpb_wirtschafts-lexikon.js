...
(async () => {
    ...
    const url = 'https://www.bpb.de/kurz-knapp/lexika/lexikon-der-wirtschaft/';
    await page.goto(url);
    ...
    for (const url of urls) {
        ...
        $('.text-content.spacer-horizontal__inset-narrow > p').each((index, element) => {
            ...
            if(!temp.includes("Quelle:") && !temp.includes("Das Lexikon als barrierefreie PDF herunterladen") && !temp.includes(" Artikel als PDF zum Download") && !temp.includes("Siehe auch:") && !temp.includes("Duden"))
                content += temp;
        });
        ...
        try {
            ...
            fs.writeFileSync(`../everyday/bpb_wirtschafts-lexikon/${i}.txt`, content);
